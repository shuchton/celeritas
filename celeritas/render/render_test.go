...
var pageData = []struct {
	...
	{"go_page_no_template", "go", "none", true, "no error when rendering non existent go template"},
	{"jet_page", "jet", "home", false, "error rendering jet template"},
	{"jet_page_no_template", "jet", "none", true, "no error when rendering non existent jet template"},
	{"invalid_render_engine", "foo", "home", true, "no error when rendering with non existent template engine"},
}
