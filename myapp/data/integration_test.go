...
func TestUser_ResetPassword(t *testing.T) {
	...
	if err == nil {
		t.Error("no error when password changed for non-existent user")
	}
}
...
func TestToken_GetUserForToken(t *testing.T) {
	...
	if err == nil {
		t.Error("no error when getting user with a bad token")
	}
...
func TestToken_Get(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to retrieve token by id: ", err)
	}
}
...
func TestToken_GetByToken(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to retrieve token by token: ", err)
	}

	_, err = models.Tokens.GetByToken("123")
	if err == nil {
		t.Error("no error when retrieving non-existent token")
	}
}
...
var authData = []struct {
	...
	{"no_user", "", "me@here.com", false, "valid token reported as invalid"},
}
...
func TestToken_BadHeader(t *testing.T) {
	...
	token, err := models.Tokens.GenerateToken(id, time.Hour*1)
	if err != nil {
		t.Error(err)
	}
